...
{
    const char      *s;     /* input */
    char            *res;   /* NULL => phase longueur ; non-NULL => phase écriture */
...
char				*handle_escape_write(char *dst, const char *src, int *i,
						int *j);
...
char				**expansion_partition_redirection(t_list *cmd, t_list *env,
						t_arr *oper);
/*
