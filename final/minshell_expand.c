...
static t_quote_state	update_quote_state(const char *str, int i,
...
static int	init_ctx_write(t_xpctx *c, const char *str, int expanded_len,
		t_list **exp)
{
	c->s = str;
...
static int	handle_quote_len(t_xpctx *c, char ch, t_quote_state new_state)
{
	if (ch != '\'' && ch != '"')
		return (0);
	if (!((c->state == QUOTE_NONE) || (c->state == QUOTE_SINGLE && ch == '\'')
...
typedef struct s_varinfo
{
	char				*name;
	int					span;
	int literal_dollar; /* 1 si ça doit produire "$" tel quel */
}						t_varinfo;

typedef struct s_expval
{
	const char *val; /* jamais NULL : "" si inconnue, "$" si invalide */
	int span;       
...
/* résout $... en (val, span).
   ...
   - variable absente => val="" mais span correct
*/
/* Étape 1 : parser le token à partir de s[i] et décider si c'est "$" littéral */
static t_varinfo	parse_varinfo_at(const char *s, int i)
{
	t_varinfo	vi;
	int			vlen;
	char		*name;
	...
	vi.span = 1;
	vi.literal_dollar = 0;
	name = extract_var_name(s + i, &vlen);
	if (!name)
	{
		vi.literal_dollar = 1; /* cas: juste '$' sans nom valide */
		return (vi);
	}
	if (name[0] == '$' && name[1] == '\0')
	{
...
/* Étape 2 : résoudre la valeur dans l'environnement (ou "$" / "") */
static t_expval	resolve_from_info(t_list *env, t_varinfo *vi)
{
	t_expval	r;
	char		*val;
...
static t_expval resolve_var_at(const char *s, t_list *env, int i)
{
    ...
    return resolve_from_info(env, &vi);
}

static int	handle_dollar_len_count(t_xpctx *c)
{
